...
export type CalloutColor = 'white' | 'gray' | 'black' | 'gradient';

import '../buttons';

const icon = svg`<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
@customElement('vs-callout')
export class Callout extends LitElement {
    @property() color: CalloutColor = 'white';
    ...
    render() {
        const classes = classMap({
            'callout': true,
            [`callout-icon`]: this.icon,
            [`callout-${this.color}`]: true,
        });

        const iconPartial = this.icon ? html`
        <div class="icon">
        ...
        <div class="${classes}">
            <header>
                ...
                <vs-close-button @click="${this.onCloseClick}"></vs-close-button>
            </header>
            <main>
                <slot></slot>
            </main>
